...
var fs = require('fs');
var filename = "webscilab7-SteveCardozo-cardos-tweets";
...
mongoose.connect('mongodb://localhost/cardos-tweets');
...
app.get("/exportDB", function(req, res) {
	Tweet.find(function (error, tweets) {
		...
		fs.access(req.query.filename+'.xml', fs.W_OK, function(error) {
			...
			fs.writeFile(req.query.filename+'.xml', output, function(error) {
